...
def extract_pauselength(input_file):
	...
	y, sr = librosa.load(silence_file)
	duration=librosa.get_duration(y=y, sr=sr)
	y, sr = librosa.load(input_file)
	duration_1 = librosa.get_duration(y=y, sr=sr)
	...
	return duration_1, pause_length, pause_number, duration
'''
record=sys.argv[1]
folder=sys.argv[2]
...
'''
df = pd.DataFrame(columns=['ID', 'file_dur', 'len_p', 'num_p', 'sum_len_p'])
full_duration, pause_length, pause_number, duration = extract_pauselength('/Users/ivansirokov/Documents/diploma/data/audio_all/trick_denoise/russir42.wav')
print(full_duration, pause_length, pause_number, duration)

data_dir = '/Users/ivansirokov/Documents/diploma/data/audio_all/zoo_denoise'

pattern = '.wav'
filtered_files = [f for f in os.listdir(data_dir) if f.endswith(pattern)]
for file in filtered_files:
	if not file.endswith('DS_Store.wav'):
		full_duration, pause_length, pause_number, duration = extract_pauselength(
			'/Users/ivansirokov/Documents/diploma/data/audio_all/zoo_denoise/' + file)
		df.loc[len(df.index)] = [file.split('.')[0], full_duration, pause_length, pause_number, duration]
df.to_csv('/Users/ivansirokov/Documents/diploma/data/audio_all/Zoo_pauses.csv')
print(df)